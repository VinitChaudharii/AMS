...
struct vin_airline
{
	...
	int seat_num;
	char email[15];
	struct vin_airline *following;
}
*begin, *stream;
struct vin_airline *dummy;
...
void details()
{
	printf("\n\t Enter your passport number (5 Digits): ");
...
void reserve(int x)
{
	...
	{
		// first user
		begin = stream = (struct vin_airline*)malloc(sizeof(struct vin_airline));
		details();
	...
	else
	{
		...
		stream->following = (struct vin_airline *)malloc(sizeof(struct vin_airline));
		stream = stream->following;
		details();
		stream->following = NULL;
...
void savefile()
{
	FILE *fpointer = fopen("vin records", "w");
	if (!fpointer)
	{
